...
mongo_insert = partial(
    insert_to_mongoDB, fmp_api=fmp_api, alpha_vantage_api=alpha_vantage_api
)
...
etoro_list = col1.file_uploader(
    "Upload your file here :file_folder:",
    key="uploaded_file",
...
manual_list = list(
    set(
        col2.text_area("Type your tickers here:", label_visibility="visible").split(
            "\n"
        )
    )
)
...
if etoro_list is not None:
    ...
    tickers.extend(
        list(
            investment_history[investment_history["Asset type"] == "Stocks"]["Details"]
    ...
    if manual_download:
        for i, x in enumerate(eval(list_tickers)):
            ...
            if i == len(eval(list_tickers)) - 1:
                st.success("All downloads are completed.", icon="💯")
        ...
        for i, x in enumerate(eval(list_tickers)):
            mongo_insert(
                collection=stock_split,
                ticker=x,
                statement="stock_split",
                second_key="date",
            )
