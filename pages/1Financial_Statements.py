...
statements_type = generate_statements_type(
    _income_collection=income_collection,
    _cash_collection=cash_collection,
    _balance_sheet_collection=balance_sheet_collection,
)
...
company_profile.update_one(
    {"index_id": f"{ticker_list_box}_{companyA_info['ipoDate']}"},
    {"$set": companyA_info},
    upsert=True,
)
...
same_sector = sorted(
    [
        i
        for i in tickers
        if read_profile(i, company_profile)[0]["sector"] == companyA_info["sector"]
    ]
)
